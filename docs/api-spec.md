...
| ステータス | 意味 | 発生条件 |
|-----------|------|---------|
| 400 | Bad Request | Deep paging 制限超過 (`page * perPage > 10000`)、`cursor` と検索条件/`page` の同時指定、不正な cursor トークン、cursor 期限切れ (PIT 失効) |
...
| パラメータ | 型 | デフォルト | 説明 |
|----------|-----|-----------|------|
| `keywords` | string | — | 検索キーワード (カンマ区切りで複数指定可)。ダブルクオートで囲むとフレーズマッチ (例: `"RNA-Seq"`) |
...
**フレーズマッチ**: `keywords` パラメータでダブルクオートで囲まれたキーワードはフレーズマッチ (ES `multi_match` の `type: "phrase"`) で検索する。トークン順序を保持した完全一致となるため、`keywords="RNA-Seq"` は "RNA-Seq" を含むエントリーのみにマッチし、"rRNA gene seq" のような部分一致を除外できる。クオートなしのキーワードは従来通りのトークンベースマッチ。混在可能: `keywords="RNA-Seq",cancer`。
