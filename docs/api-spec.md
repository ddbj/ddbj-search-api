...
- **ファセット集計**: 検索結果のファセットカウント (type, organism, accessibility 等) を取得可能
...
| GET | `/db-portal/parse` | Advanced Search DSL を JSON tree に変換 (GUI state 復元用) |
...
- 前後の空白・タブを `strip` した結果、外側のダブルクオート有無いずれも許容 (クオート内が完全一致なら可)
- ワイルドカード (`*`, `?`) を含まない
