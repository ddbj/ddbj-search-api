...
| 501 | Not Implemented | DB Portal API の未実装機能 (Advanced Search、Solr 待ち DB) |
...
- AP1 (現行): 4 パターン分岐の骨組み、ES 対応 6 DB のシンプル検索、Solr 待ち DB (`trad`, `taxonomy`) は 501、Advanced Search は 501
- AP3 (予定): Advanced Search DSL パーサ (Lark LALR(1)) で `adv` を有効化
- AP4 (予定): Solr proxy で `trad` (ARSA) / `taxonomy` (TXSearch) を有効化
...
| # | クエリ | 処理 |
|---|-------|-----|
| 1 | `q` のみ | 横断シンプル検索 (count-only、ES 対応 6 DB に順次発行、`trad`/`taxonomy` は `not_implemented` 固定) |
| 2 | `q` + `db` (ES 対応 6 DB) | DB 指定シンプル検索 (`hits` envelope + pagination) |
| 3 | `q` + `db=trad` / `db=taxonomy` | 501 (`db-not-implemented`) |
...
{
  "databases": [
    { "db": "trad",       "count": null, "error": "not_implemented" },
    ...
    { "db": "taxonomy",   "count": null, "error": "not_implemented" }
  ]
}
```
...
- `count` は `track_total_hits=true` に基づく正確値
- `error` 値: `timeout`, `upstream_5xx`, `connection_refused`, `not_implemented` (AP4 で削除), `unknown`
