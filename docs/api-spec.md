...
| `organism` | string | — | NCBI Taxonomy ID (例: `9606`)。`/facets` の `organism` bucket の `value` をそのまま渡せる (§ ファセット) |
...
| `fields` | string | — (全フィールド) | レスポンスに格納するフィールドを限定 (カンマ区切り)。ES ドキュメントのトップレベルフィールド名を指定 (例: `identifier,organism,datePublished`)。指定外のフィールドは ES から取得しないが、レスポンス schema 上のキー自体は保持され値は `null` で返る (Pydantic レスポンスモデルが必須キーを維持するため)。未指定時はすべてのフィールドを返す |
...
旧仕様では `organism` facet bucket の `value` に scientific name (`"Homo sapiens"`) を返しており、検索 API への再注入ができなかった。今回の改修で `value` は TaxID に変更され、name は `label` に分離された (**Breaking change**)。同一 TaxID で name 表記揺れがある場合、旧仕様では別 bucket としてカウントが分散していたが、新仕様では同 bucket に合算される。
