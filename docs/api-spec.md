...
### DBLinks API (関連 ID 逆引き: 2 エンドポイント)
...
| 500 | Internal Server Error | ES 接続エラー、DuckDB ファイルが見つからない (DBLinks API)、その他サーバー内部エラー |
...
`bioproject`, `biosample`, `gea`, `geo`, `hum-id`, `insdc-assembly`, `insdc-master`, `jga-dac`, `jga-dataset`, `jga-policy`, `jga-study`, `metabobank`, `pubmed-id`, `sra-analysis`, `sra-experiment`, `sra-run`, `sra-sample`, `sra-study`, `sra-submission`, `taxonomy`, `umbrella-bioproject`
...
```json
{
  "types": [
    ...
    "insdc-assembly", "insdc-master", "jga-dac", "jga-dataset",
    "jga-policy", "jga-study", "metabobank", "pubmed-id",
    "sra-analysis", "sra-experiment", "sra-run", "sra-sample",
    "sra-study", "sra-submission", "taxonomy", "umbrella-bioproject"
  ]
}
```
...
- `links` の各要素は converter 由来の `Xref` (identifier, type, url)
- `links` はタイプ昇順 → アクセッション昇順でソート (決定的)
- 該当なしの場合: 200 + 空の `links: []`
...
| `DbLinksResponse` | 関連 ID (identifier, type, links: `list[Xref]`) |
...
| `Xref` | 全タイプ共通 | 外部データベース参照 (identifier, type, url) |
