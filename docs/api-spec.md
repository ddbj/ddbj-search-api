...
**フレーズマッチ** (`keywords`): 以下のいずれかに該当するトークンは ES `multi_match` の `type: "phrase"` でフレーズマッチ検索する。トークン順序を保持した完全一致となるため、analyzer による token 分割で精度が下がるのを防ぐ:
...
**`keywordFields` allowlist**: `identifier`, `title`, `name`, `description` の 4 値のみ受け付ける (allowlist 外は 422)。
...
**nested フィールド検索** (`organization` / `publication` / `grant`): 詳細は § nested フィールド検索 を参照。これら 3 つは全 endpoint (cross-type 含む) で共通に受け付け、対応 nested path を持たない type に渡された場合は ES 側で match なしで結果 0 件化される。
...
- **term filter parameter を持つ facet** (`objectType`, `libraryStrategy`, `librarySource`, `librarySelection`, `platform`, `instrumentModel`, `libraryLayout`, `analysisType`, `experimentType`, `studyType`, `submissionType`, `datasetType`): `?<facet>=<value>` で直接再注入できる (例: `libraryStrategy` facet の value `"WGS"` を `?libraryStrategy=WGS` に再注入)。DB Portal API (`/db-portal/*`) の `q` でも再注入可 (`q=library_strategy:"WGS"`、Tier 3 allowlist の snake_case field 名)
