...
- **フィールド allowlist (Tier 1/2/3)**: Tier 1 (横断可、8 field) / Tier 2 (横断可、converter 側正規化済の共通 field、2 field) / Tier 3 (単一 DB 指定必須、39 unique / per-DB 集計 45) の 3 段構造。横断 (cross) モードで Tier 3 を使うと 400 `field-not-available-in-cross-db` (detail に候補 DB を列挙)。
  - **Tier 1 (横断可)**:
