...
- `facets` / `facetsSize` / `facetSelfExclude` (任意) を受け付ける ([§ facet 集計](#facet-集計))。横断の facet は organism / accessibility / type のみ
...
- `facetSelfExclude`: boolean (任意、既定 false)。`true` で各 facet 自身の `q` フィルタを母集団から外す self-exclusion を有効化 ([§ 集計母集団と self-exclusion](#集計母集団と-self-exclusion))
...
- `facetSelfExclude`: boolean (任意、既定 false)。`true` で各 facet 自身の `q` フィルタを母集団から外す self-exclusion を有効化 ([§ 集計母集団と self-exclusion](#集計母集団と-self-exclusion))。cursor 排他の対象外だが cursor 経路では非適用 (従来母集団で集計)
...
- `facets`: `facets` パラメータ指定時のみ非 null ([§ facet 集計](#facet-集計))。`DbPortalFacets` (`Facets` 拡張) 型。集計母集団は既定で hits と同一 query (cursor mode では cursor token に焼き込んだ query)、`facetSelfExclude=true` で各 facet 自身のフィルタを除いた母集団になる ([§ 集計母集団と self-exclusion](#集計母集団と-self-exclusion))
...
- `facets` / `facetsSize` / `facetSelfExclude` も `cursor` と併用可能 (cursor 排他の対象外)。cursor 継続時の facet 集計は cursor token に焼き込んだ query を母集団にする。`facetSelfExclude=true` を `cursor` と併用しても **cursor 経路では self-exclusion は適用されない** (cursor token に AST を持たないため、従来どおり hits と同一母集団で集計する。[§ 集計母集団と self-exclusion](#集計母集団と-self-exclusion))
- `page * perPage > 10000` は 400 (`/entries/*` 系の deep paging 制限と同じ閾値)

## facet 集計

`/db-portal/cross-search` と `/db-portal/search` は、`q` で絞った検索ヒットを母集団とする facet 集計 (値 + 件数) をレスポンスに同梱できる。db-portal の結果ページ Sidebar (NCBI 風の「値 + 件数」フィルタ) が候補値と件数を実データから得るための経路 ([db-portal/docs/search.md § Sidebar facet](https://github.com/ddbj/db-portal/blob/main/docs/search.md))。母集団は `facetSelfExclude` で切り替える ([§ 集計母集団と self-exclusion](#集計母集団と-self-exclusion))。
...
- db-portal の facet は DSL `q` で絞り、母集団を検索ヒットと同じ `status_mode` で集計する。accession 完全一致で `suppressed` が解禁される場合 ([§ データ可視性](#データ可視性-status-制御))、facet 集計にも同じ解禁が反映される (`/facets` のような public_only 固定にしない)。

### 集計母集団と self-exclusion

facet 集計の母集団は `facetSelfExclude` パラメータ (boolean, 既定 `false`) で切り替える。いずれの場合も **hits 本体 (検索結果) は常に `q` 全フィルタ適用のまま**で、`facetSelfExclude` は facet 集計の母集団にしか影響しない。

- **既定 (`facetSelfExclude` 省略 / `false`)**: 母集団 = 検索ヒットと一致 (同じ compiled query + 同じ `status_mode`)。`q` を 1 つの facet 値で絞ると、その facet の bucket はその値だけに潰れる (self-filtering)。後方互換のためこれが既定。
- **`facetSelfExclude=true`**: 各 facet `F` について、母集団から「`F` 自身に対応する `q` のフィルタだけ」を外し、他の条件 (他 facet 由来・free text・text 行・range 行・`status`) はすべて適用する。同一 facet の他候補が bucket に残り (multi-select で追加選択が可能)、かつ他 facet の絞り込みは効く (drill-down)。NCBI 風 multi-select facet の標準的な挙動。

「`F` に対応する `q` のフィルタ」は [§ facet 値の DSL 再注入](#facet-値の-dsl-再注入) の DSL field 対応で判定する (例: organism facet → `organism_id`、objectType facet → `object_type`)。`status` filter は DSL `q` とは独立に注入されるため self-exclusion の対象外で、全 facet に適用され続ける。
...
- `facetSelfExclude`: boolean (既定 `false`)。`true` で各 facet 自身の `q` フィルタを母集団から外す self-exclusion を有効化する ([§ 集計母集団と self-exclusion](#集計母集団と-self-exclusion))。

`facets` / `facetsSize` / `facetSelfExclude` は cursor とも併用可 (cursor 排他の対象外)。cursor 継続時は cursor token に焼き込んだ query を母集団に集計する (`facetSelfExclude` は cursor 経路では非適用)。
...
- **フィールド allowlist (Tier 1/2/3)**: Tier 1 (横断可) / Tier 2 (横断可、converter 側正規化済の共通 field) / Tier 3 (単一 DB 指定必須) の 3 段構造。フィールド一覧は実装 (`allowlist.py`) を SSOT とし、本仕様では各 Tier に属する代表的なフィールドを列挙する。横断 (cross) モードで Tier 3 を使うと 400 `field-not-available-in-cross-db` (detail に候補 DB を列挙)。
  - **Tier 1 (横断可)**:
    ...
    - 生物種 (taxID): `organism_id` (`eq` / `wildcard` — ES `organism.identifier` (keyword) に `term`、数値 taxID exact。Solr TXSearch は `tax_id` に直接マップ、Solr ARSA は対応 field 不在のため非対応 (cross は arm 対象外、single は 400))
...
- **バックエンド変換**:
  ...
  - ARSA: AST → edismax `q` 文字列 (フィールド名マッピング、日付は `YYYYMMDD`、number range はそのまま、`uf` で allowlist 制御)。非対応 / 固定値 field は cross / single の per-arm 簡約 (`reduce_ast_for_db`) が compile 前に AST から除くため、コンパイラには available field しか渡らない (届いた場合は実装の不変条件違反として RuntimeError)。text 型 `contains` の simple word は `(field:"w" OR field:w*)` で前方一致 (edismax メタ文字を含む語・クオート値は `field:"w"` 完全一致のみ)
...
| `field-not-available-for-db` | 400 | 単一 DB モードで指定 `db` に存在しない field 使用 (Tier 3 の他 DB 専用 field、または `publication` を `db=biosample` に)。`detail` に有効な DB を列挙 | search / parse / serialize | クエリ |
...
内部処理は `/db-portal/cross-search?q=...` / `/db-portal/search?q=...&db=<id>` のクエリ分岐と同一: パース → validate (allowlist + mode + 深さ / 日付 / 値 / FreeText 位置制約) → JSON tree 化。3 endpoint で同じ pipeline を共有し、エラー契約も両 endpoint と同一の 10 slug を共有する。
...
`/db-portal/cross-search` / `/db-portal/search` と同一の 10 slug を共有する (`unexpected-token` / `unknown-field` / `field-not-available-in-cross-db` / `field-not-available-for-db` / `invalid-date-format` / `invalid-operator-for-field` / `nest-depth-exceeded` / `missing-value` / `invalid-freetext-position` / `duplicate-freetext`、すべて 400 + `application/problem+json`)。`field-not-available-in-cross-db` は cross モードで Tier 3 field を使用した場合、`field-not-available-for-db` は single モードで指定 DB に存在しない field を使用した場合に発動する。`q` 未指定 / `db` 値不正は FastAPI 標準の 422 (`about:blank`)。
...
| `unknown-field` / `field-not-available-in-cross-db` / `field-not-available-for-db` / `invalid-operator-for-field` / `invalid-date-format` / `missing-value` / `nest-depth-exceeded` / `invalid-freetext-position` / `duplicate-freetext` | 400 | AST → validator で reject。`/db-portal/parse` と同一エラー slug を共有 |
